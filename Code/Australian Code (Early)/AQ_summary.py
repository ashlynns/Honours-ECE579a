...
fig, ax = plt.subplots() 
ax.plot(dates, Adelaide_AQ_Values, label='Adelaide')
ax.plot(dates, Brisbane_AQ_Values, label='Brisbane')
ax.plot(dates, Melbourne_AQ_Values, label='Melbourne')
ax.plot(dates, Perth_AQ_Values, label='Perth')
ax.plot(dates, Sydney_AQ_Values, label='Sydney')
plt.xlabel('Date')
plt.ylabel('Air Quality Index')
plt.title('Air Quality Index in Major Australian Cities')
...
every_mth = 10
for m, label in enumerate(ax.yaxis.get_ticklabels()):
    if m % every_mth != 0:
        label.set_visible(False)
